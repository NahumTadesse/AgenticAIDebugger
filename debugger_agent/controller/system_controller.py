"""
System Controller — orchestrates the Detection → Fix → Documentation pipeline.

Loop logic:
  - Runs up to MAX_ITERATIONS fix attempts.
  - Stops early if the fix produces no improvement (same error count).
  - Always runs Documentation at the end regardless of outcome.
"""
...
def run_debugger(code: str) -> tuple[str, dict, dict]:
    """
    Run the full debugger pipeline on `code`.

    Returns:
        (final_code, final_report, documentation)
    """
